...
exports.handle = function(e, ctx, cb) {
    ...
    var hashids = new Hashids('SKAX');
    var sns = new AWS.SNS();
    var dataIn = "";

	console.log("PRINTING EVENT = " + e);

	var decodedId = hashids.decode(e.hashedWorkflowId);
	
	//parse xml to json
	try {
		var xml = parseXml(e.dataIn);
		dataIn = xml2json(xml);
	} catch (err)
	{
		console.log(err);
	...
	dataIn = JSON.parse(e.dataIn);

	sns.publish({
	    Message: JSON.stringify(dataIn),
